...
export default function ListingPage() {
  ...
  const handleApplyFilters = async () => {
    const hasFilter = Object.values(selectedFilters).some((v) => v !== "");
    setCurrentPage(1); // ✅ Filter lagao to page 1 pe jao
    ...
    if (fetchFilteredIdeas.fulfilled.match(result)) {
      const count = result.payload?.count || 0;
      toast.success(
        count > 0 ? `${count} ideas found!` : "No ideas found for selected filters",
        { position: "top-right" }
      );
    } else {
      toast.error("Failed to apply filters. Please try again.", {
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1">
          {paginatedIdeas.length > 0 ? (
                ...
                {paginatedIdeas.map((card) =>
                  viewMode === "grid" ? (
                    ...
                    >
                      <div className="w-full md:w-[240px] h-[140px] md:h-[130px] bg-gray-200 dark:bg-white/10 relative overflow-hidden rounded-lg shrink-0">
                        <Image
                          src={`${BASE_URL}${card.image}` || "/images/startup-sketch.png"}
                          alt="startup image"
                          fill
                          className="object-cover rounded-lg"
                        />
            ...
            <div className="flex flex-col items-center justify-center py-20 px-4 text-center min-h-[500px]">
              <div className="relative w-64 h-64 mb-8">
                <Image src="/images/no-data-light2.png" alt="No ideas found" fill className="object-contain dark:hidden" />
                <Image src="/images/no-data-dark.png" alt="No ideas found" fill className="object-contain hidden dark:block" />
              </div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">No Business Ideas Yet</h2>
              <p className="text-gray-500 dark:text-gray-400 max-w-sm mx-auto text-sm leading-relaxed">
                We're working on adding high-quality business ideas. Stay tuned!
              </p>
            </div>
