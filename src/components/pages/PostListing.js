...
export default function PostListing() {
  ...
  // Redux selector to capture dynamic values
  const { posts: feedPosts = [], loading } = useSelector((state) => state.profile);

  // Trigger api implementation wrapper on component render mount
  useEffect(() => {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <Link href={"/add-new-post"}>
            <button className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-[#FB8122] hover:bg-[#e6751e] text-white rounded-full font-medium text-sm transition-all shadow-md shadow-[#FB8122]/20 hover:-translate-y-0.5">
              <Plus size={16} strokeWidth={2.5} />
              Add New Post
            </button>
          </Link>
        ...
        <div className="space-y-3 md:space-y-4">
          {loading && feedPosts.length === 0 ? (
            <div className="text-center text-sm text-icon opacity-70 py-6">Loading posts...</div>
          ) : feedPosts.length === 0 ? (
            <div className="text-center text-sm text-icon opacity-60 italic py-6">No posts found on feed.</div>
          ) : (
            feedPosts.map((post) => (
              <Link
                href={`/post-listing/${post._id}`}
                key={post._id}
                className="group bg-card border border-gray-200 dark:border-white/5 rounded-xl md:rounded-2xl p-4 md:p-5 flex items-center justify-between gap-4 shadow-sm hover:shadow-md transition-all hover:border-[#FB8122]/30 dark:hover:border-[#FB8122]/30 cursor-pointer"
              >
                {/* Post Content */}
                ...
                {/* Right Side Media and User Elements */}
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  src={`${BASE_URL}${post.image}`}
                      alt={post.title}
                    className="w-10 h-10 md:w-11 md:h-11 rounded-full object-cover border border-gray-200 dark:border-gray-700"
                    loading="lazy"
                  />
                </div>
              </Link>
