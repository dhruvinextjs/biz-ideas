...
// import { getPostDetail, resetPostState } from "@/redux/slices/PostSlice";
...
//   // Redux states injection mapping
//   const { currentPost: post, loading } = useSelector((state) => state.posts);
//   const BASE_IMAGE_URL = "http://192.168.29.108:5007";
...
//   if (loading) {
//     return (
//       <div className="min-h-screen flex items-center justify-center bg-background text-icon text-sm">
//         Loading post properties data...
//       </div>
//     );
//   }
...
//   if (!post) {
//     return (
//       <div className="min-h-screen flex flex-col items-center justify-center bg-background text-icon text-sm gap-4">
//         <span>Post not found or pending verification clearance.</span>
//         <button onClick={() => router.back()} className="text-[#FB8122] underline">Go Back</button>
//       </div>
//     );
//   }
...
//         <button 
//           onClick={() => router.back()}
//           className="flex items-center gap-2 text-sm text-gray-600 dark:text-[#BACCDE] hover:text-[#FB8122] dark:hover:text-[#FB8122] transition-colors mb-8 group"
//         >
//           <ArrowLeft
//             size={16}
//             className="group-hover:-translate-x-1 transition-transform"
...
//             <span className="opacity-90 text-icon">By</span>{" "}
//             <span className="font-semibold text-blue-600 dark:text-blue-400 cursor-pointer hover:underline">
//               <Link href={"/user-detail"}>{post.authorName || "Anonymous"}</Link>
//             </span>
//             <span className="opacity-90 text-icon">•</span>
//             <span className="opacity-90 text-icon">{formatDetailDate(post.createdAt)}</span>
//           </div>
//         </header>

//         {/* ========================================== */}
//         {/* 3. HERO IMAGE TAG IMPLEMENTATION           */}
//         {/* ========================================== */}
//         <div className="w-full aspect-[16/9] md:aspect-[18/10] bg-gray-200 dark:bg-[#D9D9D9] rounded-4xl mb-10 overflow-hidden shadow-sm">
//           {post.image ? (
//             <img 
//               src={`${BASE_IMAGE_URL}${post.image}`} 
//               alt={post.title} 
//               className="w-full h-full object-cover"
//               loading="eager"
//             />
//           ) : (
//             <div className="w-full h-full bg-gray-300 dark:bg-gray-700 flex items-center justify-center text-gray-500 text-xs italic">
//               No cover media attached
//             </div>
//           )}
//         </div>
...
//         {/* ========================================== */}
//         <article className="prose prose-gray dark:prose-invert max-w-none text-icon text-base md:text-[14px] leading-relaxed mb-12 space-y-6">
//           <p className="text-icon whitespace-pre-wrap">
//             {post.content}
//           </p>
//         </article>
...
//               className="w-full bg-white dark:bg-[#1D2659] border border-[#E6E6E6] dark:border-[#3E4A92] rounded-lg p-4 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#FB8122]/50 transition-all resize-none mb-4 shadow-sm"
//             ></textarea>
//             <button className="px-8 py-3 bg-[#FB8122] hover:bg-[#e6751e] text-white rounded-full font-medium text-xs transition-all shadow-md shadow-[#FB8122]/20">
//               POST COMMENT
//             </button>
//           </div>
...
//                       </span>
//                       <span className="text-xs text-gray-500 dark:text-[#BACCDE]">
//                         {formatDetailDate(comment.createdAt) || "Recent"}
//                       </span>
//                     </div>
//                     <p className="text-sm text-icon opacity-90 mb-2 leading-relaxed">
...
//               ))
//             ) : (
//               <p className="text-xs text-icon opacity-60 italic py-2">No comments on this post yet.</p>
//             )}
//           </div>
//         </section>
//       </div>
...
// import { useParams, useRouter } from "next/navigation";
// import { useDispatch, useSelector } from "react-redux";
// import { getPostDetail, resetPostState, upvotePost } from "@/redux/slices/PostSlice"; // Import upvotePost
...
//   // ✅ New: Likes Trigger handler
//   const handleLikeClick = () => {
//     if (params?.id) {
//       dispatch(upvotePost(params.id));
//     }
//   };
...
//         {/* ========================================== */}
//         <div className="flex items-center gap-6 border-t border-b border-gray-200 dark:border-white/10 py-4 mb-12">
//           {/* ✅ CLICK EVENT ATTACHED & COUNT VALUE MADE DYNAMIC */}
//           <button 
//             onClick={handleLikeClick}
//             className="flex items-center gap-2 border border-gray-600 dark:border-[#363B57] rounded-4xl py-2 px-4 text-xs text-icon hover:text-[#FB8122] transition-colors"
//           >
//             <ThumbsUp size={18} />
//             <span>{post.upvotes || 0} Likes</span>
//           </button>
...
//   // ✅ States for clean local toggling
...
//   // ✅ Sync initial count ONLY ONCE when post properties load for the first time
//   useEffect(() => {
//     if (post && !isInitialLoaded) {
//       setLikesCount(post.upvotes || 0);
//       setIsInitialLoaded(true); // Locks backend overwrite from disturbing local toggle clicks
//     }
//   }, [post, isInitialLoaded]);

//   // ✅ Fixed Likes Trigger: 1st click +1 increment, 2nd click -1 decrement strictly managed
//   const handleLikeClick = () => {
//     if (params?.id) {
//       if (!isLiked) {
...
//       // Dispatch API action to log action inside database
//       dispatch(upvotePost(params.id));
//     }
//   };
...
//           <button 
//             onClick={handleLikeClick}
//             className={`flex items-center gap-2 border rounded-4xl py-2 px-4 text-xs transition-colors ${
//               isLiked 
//                 ? "border-[#FB8122] text-[#FB8122]" 
//                 : "border-gray-600 dark:border-[#363B57] text-icon hover:text-[#FB8122]"
//             }`}
//           >
...
//       // ✅ Refresh protection: Load exact highlight state from localStorage on mount
//       const savedLikeStatus = localStorage.getItem(`liked_post_${params.id}`);
...
//   // ✅ Sync initial count dynamically but protect user's custom changes on refresh
//   useEffect(() => {
//     if (post && !isInitialLoaded) {
//       const savedCount = localStorage.getItem(`likes_count_${params?.id}`);

//       if (savedCount !== null) {
//         // If user already modified state before refresh, load that specific count
//         setLikesCount(Number(savedCount));
//       } else {
//         // Otherwise use standard fallback database count
//         setLikesCount(post.upvotes || 0);
//       }
//       setIsInitialLoaded(true); 
//     }
//   }, [post, isInitialLoaded, params?.id]);

//   // ✅ Strict Actions Trigger: State ONLY updates when user clicks deliberately
//   const handleLikeClick = () => {
//     if (params?.id) {
//       let updatedCount = likesCount;
//       let updatedLikedState = !isLiked;

//       if (!isLiked) {
//         updatedCount = likesCount + 1;
//         localStorage.setItem(`liked_post_${params.id}`, "true");
//       } else {
//         updatedCount = Math.max(0, likesCount - 1);
//         localStorage.removeItem(`liked_post_${params.id}`);
//       }
...
//       // ✅ Lock updated state value into storage so refresh doesn't change it
//       localStorage.setItem(`likes_count_${params.id}`, String(updatedCount));

//       // Dispatch API action to keep DB updated
//       dispatch(upvotePost(params.id));
//     }
//   };
...
// import { useParams, useRouter } from "next/navigation";
// import { useDispatch, useSelector } from "react-redux";
// import { getPostDetail, resetPostState, upvotePost, createComment } from "@/redux/slices/PostSlice"; 
...
//   // Redux states injection mapping
//   const { currentPost: post, loading, commentLoading } = useSelector((state) => state.posts);
//   const BASE_IMAGE_URL = "http://192.168.29.108:5007";
...
//   // ✅ NEW: State for text comment interaction
...
//   // ✅ NEW: Submit Comment handler with dynamic params payload mapping
//   const handleCommentSubmit = () => {
//     if (!commentText.trim() || !params?.id) return;

//     const payload = {
//       itemId: params.id,
//       itemType: "post", // Hardcoded parameter value layout requirements
//       text: commentText.trim()
//     };

//     dispatch(createComment(payload)).then((res) => {
//       if (res.meta.requestStatus === "fulfilled") {
//         setCommentText(""); // Empty text area content after dynamic addition success
//       }
//     });
//   };
...
//           {/* Comment Input */}
//           <div className="mb-10">
//             {/* ✅ Value & OnChange attributes mapped dynamically */}
//             <textarea
//               rows="3"
//               value={commentText}
...
//               className="px-8 py-3 bg-[#FB8122] hover:bg-[#e6751e] text-white rounded-full font-medium text-xs transition-all shadow-md shadow-[#FB8122]/20 disabled:opacity-50"
//             >
...
import { useParams, useRouter } from "next/navigation";
import { useDispatch, useSelector } from "react-redux";
import { getPostDetail, resetPostState, upvotePost, createComment, getPostComments, bookmarkPost } from "@/redux/slices/PostSlice";
...
export default function PostDetail() {
  ...
  // Redux states injection mapping
  const { currentPost: post, loading, commentLoading, bookmarkLoading } = useSelector((state) => state.posts);
  ...
  // State for text comment interaction
  const [commentText, setCommentText] = useState("");

  useEffect(() => {
  if (params?.id) {
    dispatch(getPostDetail(params.id));
    dispatch(getPostComments({ itemId: params.id, itemType: "post" }));

    const savedLikeStatus = localStorage.getItem(`liked_post_${params.id}`);
    if (savedLikeStatus === "true") setIsLiked(true);

    // ← Yeh add karo
    const savedBookmarkStatus = localStorage.getItem(`bookmarked_post_${params.id}`);
    if (savedBookmarkStatus === "true") setIsBookmarked(true);
  }
  return () => {
    dispatch(resetPostState());
  };
  ...
  // Submit Comment handler with dynamic params payload mapping
  const handleCommentSubmit = () => {
    if (!commentText.trim() || !params?.id) return;
    ...
    dispatch(createComment(payload)).then((res) => {
      if (res.meta.requestStatus === "fulfilled") {
        setCommentText(""); // Empty textarea text area content
        // ✅ Re-fetch full comments layout list instantly to verify author object population
        dispatch(getPostComments({ itemId: params.id, itemType: "post" }));
      }
    });
  };
...
const handleBookmark = () => {
  ...
  dispatch(bookmarkPost(params.id)).then((res) => {
    if (res.meta.requestStatus === "fulfilled") {
      const { bookmarked, message } = res.payload;
      setIsBookmarked(bookmarked);

      // ← Yeh add karo
      if (bookmarked) {
        localStorage.setItem(`bookmarked_post_${params.id}`, "true");
      } else {
        localStorage.removeItem(`bookmarked_post_${params.id}`);
      }
    ...
    <div className="min-h-screen bg-background font-sans transition-colors duration-300 py-10 px-4 md:px-8 mt-20">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-6 border-t border-b border-gray-200 dark:border-white/10 py-4 mb-12">
          <button
            onClick={handleLikeClick}
            className={`flex items-center gap-2 border rounded-4xl py-2 px-4 text-xs transition-colors ${isLiked
                ? "border-[#FB8122] text-[#FB8122]"
                : "border-gray-600 dark:border-[#363B57] text-icon hover:text-[#FB8122]"
              }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 border rounded-4xl py-2 px-6 text-xs transition-colors ${isBookmarked
                ? "border-[#FB8122] text-[#FB8122]"
                : "border-gray-600 dark:border-[#363B57] text-icon hover:text-[#FB8122]"
              }`}
          >
        ...
        <section>
          ...
          <div className="space-y-8">
            {post.comments && post.comments.length > 0 ? (
              post.comments.map((comment, index) => {
                // ✅ Extract user name string based on conditional object tree format inside response
                const dynamicAuthorName = typeof comment.user === 'object' ? comment.user?.name : (comment.authorName || "User");

                return (
                  <div key={comment._id || index} className="flex gap-4">
                    <div className="w-10 h-10 rounded-full bg-[#222B62] text-white flex items-center justify-center font-bold text-sm shrink-0">
                      {dynamicAuthorName ? dynamicAuthorName.substring(0, 2).toUpperCase() : "U"}
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-semibold text-gray-900 dark:text-white text-md">
