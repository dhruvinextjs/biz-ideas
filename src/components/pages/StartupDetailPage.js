...
import {
  ...
  postComment,
  postReply,
} from "@/redux/slices/StartupideasSlices";
...
// ✅ Like persistence helpers — businessideasDetails.js jaisa same pattern
const LIKED_IDEAS_KEY = "likedStartupIdeasList";
...
export default function DetailPage() {
  ...
  const {
    ...
    likeLoading,
    replyLoading,
  } = useSelector((state) => state.startupIdeas);
  ...
  useEffect(() => {
    if (id) {
      dispatch(fetchStartupIdeaById(id));
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (submitContactForm.fulfilled.match(result)) {
      ...
      setFormData({
        ...
        projectRequirement: "", captchaAnswer: "",
      });
      dispatch(fetchCaptcha());
    } else {
      const errMsg =
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center text-foreground">
        <p className="text-lg font-medium animate-pulse">Loading Startup Details...</p>
      </div>
    );
  }
  ...
  if (!currentIdea) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center text-foreground">
        <p className="text-lg text-gray-500">No Details Found</p>
      </div>
    );
  }
  ...
  const displayImage = currentIdea.image?.startsWith("http")
    ? currentIdea.image
    : `${BASE_URL.replace(/\/$/, "")}${currentIdea.image}`;
    ...
    <div className="min-h-screen bg-background text-foreground font-sans transition-colors duration-300 pb-20">
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 items-center justify-between">
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-3">
              <span className="flex items-center gap-2 text-icon bg-gray-300 dark:bg-[#3E3F65] px-4 py-2 rounded-md text-sm font-normal capitalize">
                <PiSquaresFourBold size={16} className="text-icon" /> {currentIdea.category || currentIdea.type || "Local Business"}
              </span>
          ...
          <div className="w-full lg:w-[450px] shrink-0 h-[280px] relative rounded-2xl overflow-hidden shadow-lg border border-gray-200 dark:border-white/5">
            <Image
              ...
              unoptimized={currentIdea.image ? true : false}
            />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 mt-12 flex flex-col lg:flex-row gap-12">
        ...
        <div className="flex-1 space-y-10">
          ...
          <section>
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">Tech Stack & Tools Required</h2>
            <div className="w-full border-t-2 border-dashed border-gray-300 dark:border-[#1C234D] mb-6"></div>
            {currentIdea.tags && currentIdea.tags.length > 0 && !currentIdea.tags[0].includes("dummy") ? (
              <div className="flex flex-wrap gap-2 mb-4">
                {currentIdea.tags.map((tag, idx) => (
                  <span key={idx} className="bg-gray-200 dark:bg-[#1D2659] text-xs px-3 py-1 rounded-full text-gray-700 dark:text-white">{tag}</span>
                ))}
              </div>
            ) : null}
          ...
          <div className="flex flex-wrap gap-4 pb-8 border-b border-gray-200 dark:border-white/10">
            ...
            <button onClick={handleDownloadClick} className="flex items-center gap-2 text-white bg-[#2A357B] hover:bg-[#232e66] px-6 py-2 text-sm font-semibold transition-colors rounded-3xl shadow-lg shadow-blue-900/20">
              <Download size={16} /> Download PDF
            </button>
          </div>

          {/* ✅ Discussions — fully dynamic */}
          <section className="pt-0">
            ...
            <div className="mb-8">
              ...
              <button
                ...
                className="bg-primary text-white px-8 py-3 rounded-full text-sm font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
          ...
          <div className="dark:bg-[#1A2342] dark:bg-card rounded-xl p-6 shadow-xl sticky top-24 border border-transparent dark:border-white/5">
            ...
            <form className="space-y-4" onSubmit={handleFormSubmit}>
              <div>
                ...
                <input type="text" name="name" value={formData.name} onChange={handleInputChange} placeholder="Type here..." className="w-full bg-gray-200 dark:bg-[#1D2659] border border-transparent dark:border-[#3E4A92] rounded-md p-2.5 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-primary" required />
              </div>
              <div>
                <label className="block text-sm font-normal text-icon mb-2">Email <span className="text-red-500">*</span></label>
                <input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="example@mail.com" className="w-full bg-gray-200 dark:bg-[#1D2659] border border-transparent dark:border-[#3E4A92] rounded-md p-2.5 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-primary" required />
              </div>
              <div>
                <label className="block text-sm font-normal text-icon mb-2">Country <span className="text-red-500">*</span></label>
                <select name="country" value={formData.country} onChange={handleInputChange} className="w-full bg-gray-200 dark:bg-[#1D2659] border border-transparent dark:border-[#3E4A92] rounded-md p-2.5 text-sm text-icon focus:outline-none focus:border-primary" required>
                  <option value="">Select</option>
                  <option value="United States">United States</option>
                  <option value="India">India</option>
                  <option value="United Kingdom">United Kingdom</option>
              ...
              <div>
                ...
                <input type="tel" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="Type here..." className="w-full bg-gray-200 dark:bg-[#1D2659] border border-transparent dark:border-[#3E4A92] rounded-md p-2.5 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-primary" required />
              </div>
              <div>
                <label className="block text-sm font-normal text-icon mb-2">Project Requirement (optional)</label>
                <textarea name="projectRequirement" value={formData.projectRequirement} onChange={handleInputChange} placeholder="Type here..." rows="3" className="w-full bg-gray-200 dark:bg-[#1D2659] border border-transparent dark:border-white/10 rounded-md p-2.5 text-sm text-white placeholder-gray-400 focus:outline-none focus:border-primary resize-none"></textarea>
              </div>

              <div>
                <div className="bg-white w-50 border border-gray-300 p-3 flex flex-col sm:flex-nowrap">
                  <label className="block text-sm font-semibold text-black">Captcha <span className="text-red-500">*</span></label>
                  <div className="flex flex-row items-center gap-3">
                    <span className="text-sm text-black whitespace-nowrap">
                      {captchaData?.question ? `What is ${captchaData.question} ?` : "Loading..."}
                    </span>
                    <input type="text" name="captchaAnswer" value={formData.captchaAnswer} onChange={handleInputChange} className="w-16 h-9 border border-gray-400 px-2 py-1 text-sm text-gray-900 focus:outline-none" required />
                  </div>
                </div>
              </div>
